...
public class ValidateHelper {
    ...
    public void pressSpace(By element) {
        driver.findElement(element).sendKeys(Keys.SPACE);
        ExtentTestManager.logMessage(Status.PASS, "Press enter : " + element);
    }


    public void pressTabMany() throws AWTException {
        for (int i = 0; i < 3; i++) {
    ...
    public void ZoomBrowser() {
        JavascriptExecutor jse = (JavascriptExecutor) driver;
        jse.executeScript("document.body.style.zoom = '90%';");
    }

    public void LinkTC(String url) {
        //ExtentTestManager.logMessage(Status.INFO,"Link TC: "+ url);
        ExtentTestManager.logMessage(Status.INFO, "Test Case : <a href=" + url + ">Link</a>");
    }
